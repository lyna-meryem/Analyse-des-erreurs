...
# Dates
min_date, max_date = df["[FK] Flight date"].min(), df["[FK] Flight date"].max()
date_range = st.sidebar.slider("Sélectionner la période",
...
if len(df_filtered) > 0:
    ...
    Q1, Q3 = df_filtered[selected_delta_col].quantile([0.25, 0.75])
    IQR = Q3 - Q1
